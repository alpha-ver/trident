namespace :leon do
  ...
  task :upd_sports => :environment do
    ...
    sports[:rows].each do |sport|
      model_sport = Sport.find_by(:api_leon_id => sport[:id])
      if model_sport.nil?
        ...
        if model_sport.nil?
          ...
          model_sport = Sport.new(
            ...
            created: api.name,
            enable: true,
            api_leon_id: sport[:id],
            api_leon_params: sport
          )
          model_sport.save
        else
          puts "Sport \"#{sport[:name]}\" merge.".colorize(:yellow)
          model_sport.api_leon_id = sport[:id]
          model_sport.api_leon_params = sport
          model_sport.save
  ...
  task :upd_champs => :environment do
    ...
    api = Api_leon.new()
    Sport.leons.each do |model_sport|
      puts "Sport selected:#{model_sport.name}.".colorize(:green)
      champs = api.champs(model_sport.api_leon_id)

      champs[:rows].each do |champ|
        #{:id=>10150305, :count=>3, :name=>"Австралия - А Лига 2016/17"}
        model_champ = model_sport.champs.find_by(:api_leon_id => champ[:id])
        if model_champ.nil?
          ...
          if model_champ.nil?
            model_champ = Champ.new(
              ...
              api_leon_id: champ[:id],
              api_leon_params: champ
            )
            ...
            model_champ.api_leon_id = champ[:id]
            model_champ.api_leon_params = champ
            model_champ.save
          end
        else
